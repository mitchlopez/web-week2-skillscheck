...
// example = [10,300,4,321,6,87,101,100,210]
...
// console.log(bigOrSmall(example))
//////////////////PROBLEM 13////////////////////
...
// console.log(arrayReverser(Example))
//////////////////PROBLEM 14 - 18 Setup////////////////////
...
let doubled = myNumbers.map(function(cv){
	return cv *= 2;
})
...
let filtered = myNumbers.filter(function(cv){
	return cv > 100;
})
...
let forTheLoveOfGeorge = notGeorge.map(function(cv,index,arr){
	if (cv !== 'George'){
		return 'George'
	}
	return arr;
...
let enemies = people.filter(function(cv,index,arr){
	return cv.friend !== true;
})
...
var totallyAwesome = people.reduce(function(sum,cv,arr){
	return sum + cv.awesomeLevel;
}, 0) 